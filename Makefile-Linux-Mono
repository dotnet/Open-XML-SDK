...
OPENXMLDLL=OpenXMLLib.dll

# Output

FILE_DLL=$(OFAPIBLDDIR)/$(OPENXMLDLL)


# Documentation File

DOC=$(OFAPIBLDDIR)/$(OPENXMLDLLDOC)
...
FILES_SRC=\
    ...
    \
	$(OFAPIDIR)/Properties/AssemblyInfo.cs.source \
    \
...
$(FILE_DLL): $(EXCEPTIONMESSAGESRES).resources $(VALIDATIONRESOURCESRES).resources
	mkdir -p $(OFAPIBLDDIR)
...
clean:
	rm -rf $(OFAPIBLDDIR)
	rm -f $(EXCEPTIONMESSAGESRES).resources
	rm -f $(VALIDATIONRESOURCESRES).resources