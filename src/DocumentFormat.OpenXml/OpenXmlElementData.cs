...
namespace DocumentFormat.OpenXml
{
    internal class OpenXmlElementData
    {
        ...
        {
            _rawAttributes = new Lazy<ReadOnlyArray<ElementProperty<OpenXmlSimpleType>>>(() => ElementPropertyCollection.GetProperties(cache, type), true);
            _rawElements = new Lazy<ReadOnlyArray<ElementProperty<OpenXmlElement>>>(() => ElementPropertyCollection.GetElements(cache, type), true);
            _info = new Lazy<ElementTypeInfo>(() => ElementTypeInfo.Create(type), true);
            _lookup = new Lazy<ElementSchemaLookup>(() => ElementSchemaLookup.CreateLookup(type, cache), true);
        }
