            ...
            if (value is not null && !value.IsValid)
            {
                ...
                if (!value.IsEnum && string.IsNullOrEmpty(value.InnerText))
                {
                    context.CreateError(
                        ...
                        description: SR.Format(message, current.Property.QName.ToXmlQualifiedName(), current.Value.InnerText, ValidationResources.Sch_EmptyAttributeValue),
                ...
                else
                {
                    context.CreateError(
                        id: errorMessageResourceId,
                        description: SR.Format(message, current.Property.QName.ToXmlQualifiedName(), value, ValidationResources.Sch_EnumerationConstraintFailed),
                        errorType: ValidationErrorType.Schema);
                }
            }
        }
