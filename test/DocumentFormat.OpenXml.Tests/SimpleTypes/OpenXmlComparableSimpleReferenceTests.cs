...
namespace DocumentFormat.OpenXml.Tests.SimpleTypes
{
        ...
        where T : class, IComparable, IComparable<T>, IEquatable<T>
    {
        ...
        protected OpenXmlComparableSimpleReference<T>? NullReference1 { get; } = null;

        protected OpenXmlComparableSimpleReference<T>? NullReference2 { get; } = null;
        ...
        {
            Assert.Equal(SmallValue1, SmallValue2);
            Assert.Equal(NullValue1, NullValue2);
            Assert.Equal(NullReference1, NullReference2);

            Assert.NotEqual(SmallValue1, LargeValue);
            Assert.NotEqual(SmallValue1, NullValue1);
            Assert.NotEqual(NullReference1, SmallValue1);
            Assert.NotEqual(NullReference1, LargeValue);
            Assert.NotEqual(NullReference1, NullValue1);

            Assert.True(SmallValue1 <= SmallValue2);
            Assert.True(SmallValue1 <= LargeValue);
            Assert.True(NullReference1 <= SmallValue2);
            Assert.True(NullReference1 <= LargeValue);
            ...
            Assert.True(SmallValue2 >= NullReference1);
            Assert.True(LargeValue >= NullReference1);
