...
namespace DocumentFormat.OpenXml
{
    internal static partial class NamespaceIdMap
    {
        ...
        // The ID is used by generator. Consider re-build the generater and re-generate the DOM classes.
        // !! CAUTION !!
        ...
        // support. Before we try to get the index of a namespace, we check if it's in this list to rename to the exptected correct namespace.
        ...
        public static int Count
        {
            get
            {
                return _namespaceList.Length;
            }
        }

        //public UInt32 Id ( string tagNamespace 
        public static byte GetNamespaceId(string namespaceUri)
        {
            ...
            if (index < 0)
            {
                // Check if namespaceUri is in the obsolete namespace dic.
                if (_extendedNamespaceDic.TryGetValue(namespaceUri, out namespaceUri))
                {
                    index = Array.IndexOf(_namespaceList, namespaceUri);
                }
            }
        ...
        {
            return (string)_namespaceList[namespaceId];
        }
